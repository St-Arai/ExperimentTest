...
public class HelpCommand extends VanillaCommand {
	...
	public boolean execute(CommandSender sender, String currentAlias, String[] args) {
		...
		ChatPaginator.ChatPage page = ChatPaginator.paginate(topic.getFullText(sender), pageNumber,
	...
	public List<String> tabComplete(CommandSender sender, String alias, String[] args) {
		...
		if (args.length == 1) {
			...
			for (HelpTopic topic : Bukkit.getServer().getHelpMap().getHelpTopics()) {
				String trimmedTopic = topic.getName().startsWith("/") ? topic.getName()
	...
	protected HelpTopic findPossibleMatches(String searchString) {
		...
		for (HelpTopic topic : Bukkit.getServer().getHelpMap().getHelpTopics()) {
			String trimmedTopic = topic.getName().startsWith("/")
			...
			if (Character.toLowerCase(trimmedTopic.charAt(0)) != Character.toLowerCase(searchString
					.charAt(0))) {
				continue;
			}
		...
		if (possibleMatches.size() > 0) {
			return new IndexHelpTopic("Search", null, null, possibleMatches, "Search for: "
					+ searchString);
		} else {
			return null;
		}
	 ...
	 * <a href="http://en.wikipedia.org/wiki/Damerau–Levenshtein_distance">Wikipedia:
	 ...
	 * @return The number of substitutions, deletions, insertions, and transpositions required to
	 *         get from s1 to s2.
	 */
	protected static int damerauLevenshteinDistance(String s1, String s2) {
		...
		for (int i = 1; i <= s1Len; i++) {
			...
			for (int j = 1; j <= s2Len; j++) {
				...
				H[i + 1][j + 1] = Math.min(H[i + 1][j + 1], H[i1][j1] + (i - i1 - 1) + 1
						+ (j - j1 - 1));
			}
