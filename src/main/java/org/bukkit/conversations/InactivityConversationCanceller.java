...
/**
 * An InactivityConversationCanceller will cancel a {@link Conversation} after a period of
...
public class InactivityConversationCanceller implements ConversationCanceller {
			...
			public void run() {
				if (conversation.getState() == Conversation.ConversationState.UNSTARTED) {
					...
					conversation.abandon(new ConversationAbandonedEvent(conversation,
							InactivityConversationCanceller.this));
				}
			}
		}, timeoutSeconds * 20L);
	}
	...
	/**
	 * Subclasses of InactivityConversationCanceller can override this method to take additional
	 * actions when the inactivity timer abandons the conversation.
	 * 
	 * @param conversation
