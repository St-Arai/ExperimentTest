...
/**
 * The LazyMetadataValue class implements a type of metadata that is not computed until another
 * plugin asks for it. By making metadata values lazy, no computation is done by the providing
 * plugin until absolutely necessary (if ever). Additionally, LazyMetadataValue objects cache their
 * values internally unless overridden by a {@link CacheStrategy} or invalidated at the individual
 * or plugin level. Once invalidated, the LazyMetadataValue will recompute its value when asked.
 */
public class LazyMetadataValue extends MetadataValueAdapter {
	...
	/**
	 * Initialized a LazyMetadataValue object with the default CACHE_AFTER_FIRST_EVAL cache
	 * strategy.
	 * 
	 * @param owningPlugin
	 *            the {@link Plugin} that created this metadata value.
	 * @param lazyValue
	 ...
	 *            the lazy value assigned to this metadata value.
	 */
	public LazyMetadataValue(Plugin owningPlugin, CacheStrategy cacheStrategy,
	 ...
	 *             if computing the metadata value fails.
	 */
	...
	public enum CacheStrategy {
		/**
		 * Once the metadata value has been evaluated, do not re-evaluate the value until it is
		...
		/**
		 * Once the metadata value has been evaluated, do not re-evaluate the value in spite of
		 * manual invalidation.
		 */
		CACHE_ETERNALLY
	}
