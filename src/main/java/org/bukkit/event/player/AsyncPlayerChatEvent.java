...
/**
 * This event will sometimes fire synchronously, depending on how it was triggered. The constructor
 * provides a boolean to indicate if the event was fired synchronously or asynchronously. When
 * asynchronous, this event can be called from any thread, but the main thread, and has limited
 * access to the API.<br>
 * <br>
 * If a player is the direct cause of this event by incoming packet, this event will be
 * asynchronous. If a plugin triggers this event by compelling a player to chat, this event will be
 * synchronous.<br>
 * <br>
...
public class AsyncPlayerChatEvent extends PlayerEvent implements Cancellable {
	...
	public void setFormat(String format) throws IllegalFormatException, NullPointerException {
		...
		try {
			format = String.format(format, player, message);
	...
	/**
	 * Gets a set of recipients that this chat message will be displayed to. The set returned is not
	 * guaranteed to be mutable and may auto-populate on access. Any listener accessing the returned
	 * set should be aware that it may reduce performance for a lazy set implementation.<br>
	 * <br>
	 * <b>Listeners should be aware that modifying the list may throw
	 * {@link UnsupportedOperationException} if the event caller provides an unmodifiable set.</b>
	 * 
