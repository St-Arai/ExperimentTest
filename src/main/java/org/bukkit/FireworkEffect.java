...
public final class FireworkEffect implements ConfigurationSerializable {
	...
	public static final class Builder {
		...
		/**
		 * Create a {@link FireworkEffect} from the current contents of this builder. To
		 * successfully build, you must have specified at least one color.
		 * 
		 * @return The representative firework effect
		 */
		public FireworkEffect build() {
			return new FireworkEffect(flicker, trail, colors.build(), fadeColors == null
					? ImmutableList.<Color> of()
					: fadeColors.build(), type);
		}
	}
	...
	FireworkEffect(boolean flicker, boolean trail, ImmutableList<Color> colors,
			ImmutableList<Color> fadeColors, Type type) {
		if (colors.isEmpty()) {
	...
	public static ConfigurationSerializable deserialize(Map<String, Object> map) {
		...
		return builder().flicker((Boolean) map.get(FLICKER)).trail((Boolean) map.get(TRAIL))
				...
				.withFade((Iterable<?>) map.get(FADE_COLORS)).with(type).build();
	}

	public Map<String, Object> serialize() {
		return ImmutableMap.<String, Object> of(FLICKER, flicker, TRAIL, trail, COLORS, colors,
				FADE_COLORS, fadeColors, TYPE, type.name());
	}
